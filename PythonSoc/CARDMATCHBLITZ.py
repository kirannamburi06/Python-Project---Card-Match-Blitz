...
while True:
    ...
    if state == HOME:
        ...
        for k,v in clickedCards.items():
            screen.blit(x.cardShuf[k], v)

        if len(clickedCards) == 2:
            firstKey = list(clickedCards.keys())[0]
            secondKey = list(clickedCards.keys())[1]
            if x.cardShuf[firstKey] == x.cardShuf[secondKey]:
                yesReactions()
            else:
                shuffle2()
                clickedCards.clear()
